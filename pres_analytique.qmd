...
&:= \sum_{k \in S} \frac{u^\text{lin}_k(\theta)}{\pi_k}
\end{align}

- La variable $\{ u^\text{lin}_k(\theta) \}$ est la variable linéarisée associée à l'estimateur $\hat{\theta}$ $\to$ $\theta$ est inconnu.

- La variable $\{ u^\text{lin}_k(\hat{\theta}) \}$ est la variable linéarisée estimée associée à l'estimateur.
