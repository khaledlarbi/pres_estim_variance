...
#Ici, j'ai pris la variable d'intérêt mais j'aurais pu générer une variable.
...
Si certains individus renseignés dans le paramètre `reference_id` de
la fonction `define_variance_wrapper` ne sont pas dans la table
`var_interet` alors le message d’avertissement suivant apparaître :
...
Les estimations de variance seront très probablement fausses. Il est recom-
mandé de comparer les tables utilisées lors de la création de variance
wrapper et celles contenant les variables d’intérêt
...
Rappel sur les statistic wrapper


#### $\displaystyle \widehat{\mathbb{V}}(\hat{t}_{y,\text{cal}})$
...
#### 
REMARQUE : MEAN = ESTIMATEUR D'HAYEK 
